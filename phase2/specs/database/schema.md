...
This specification defines the SQLModel database schema for the Todo App. The database uses Neon PostgreSQL with two primary tables: `users` and `tasks`. All table names are plural and lowercase following conventions. User isolation is enforced through `user_id` foreign key constraints on the `tasks` table.
...
- Foreign key cascade delete is acceptable; deleting user deletes all tasks automatically
